...
class Camera:
    ...
    def __init__(self, width, height):
        ...
        self.ppcImgMem = ct.pointer(
            (ct.c_char * ((width * np.int((self.color_depth.value + 7) / 8) + self.adjust) * height))())
        ...
        nSizeOfParam = ct.sizeof(IMAGE_FORMAT_LIST) + ct.sizeof(
            IMAGE_FORMAT_INFO * (self.number_of_image_formats.value - 1))
        ...
        self.pcMem = ct.pointer(
            (ct.c_char * ((self.width * np.int((self.color_depth.value + 7) / 8) + self.adjust) * self.height))())
